...
| CHARACTER-002 | p11 | 새 캐릭터 → 4개 Style 중 하나; 이후 해당 고유 효과 적용 | 캐릭터 생성·행동 | 캐릭터 생성, `rulesEngine.ts` | Partial | 없음 | Style 선택은 있으나 Soaring의 수로 안전 등 룰북 밖 효과가 섞여 있다. |
...
| CHARACTER-005 | p14-15 | Familiar → d12 종류 또는 선택; 각 동물의 명시 효과만 적용 | 캐릭터·동료 | familiar data, action handlers | Partial | 없음 | 12종 생성은 있으나 일부 효과가 누락되고 비행/방수 능력이 추가됐다. |
...
| TRAVEL-002 | p22 | 수로 이동 → 이동 수단/규칙을 충족; 젖음과 장비 보호 적용 | 이동 | `travelEngine.ts`, travel adapter | Partial | `gameplayEngine.test.ts [TRAVEL-002]` | 젖음과 물품 손실은 엔진 처리하지만 UI가 선택한 전체 route를 waterway로 표시하는 단순화가 남아 있다. |
...
| TRAVEL-008 | p25 | 현재 계절에 맞는 지역/Soar 조우를 카드 규칙대로 선택 | 이동·조우 | canonical encounter selector | Exact | `gameplayEngine.test.ts`, `phase3Engine.test.ts [TRAVEL-008]` | 103개 행의 지역·계절·카드 key와 경고 9행을 원문 기준으로 선택한다. Printed effect 실행 완전성은 `TRAVEL-009`에서 별도 추적한다. |
| TRAVEL-009 | p25 | 조우의 강제 효과·선택·자원 변화를 해소한 뒤 다음 단계 | 조우 모달 | `encounterEngine.ts` | Partial | `gameplayEngine.test.ts [TRAVEL-009]` | 구조화 효과·선택·중복 방지는 실행되며 미전사 printed effect는 명시적 manual 상태다. |
...
| Rule ID | 룰북 | 규칙 명세 | 대응 화면 | 구현 위치 | 상태 | 자동 테스트 | 문제 |
|---|---|---|---|---|---|---|---|
| FORAGE-001 | p32 | Wild/Titan Ruin/Barrow에서만 현재 또는 인접 지역을 채집 | 채집 | `foragingEngine.ts` | Partial | `gameplayEngine.test.ts [FORAGE-001]` | 현재 위치 유형은 강제하지만 인접 지역 선택을 지도 edge와 대조하는 입력은 아직 없다. |
...
| FORAGE-006 | p33 | 채집 뒤 해당 지역·계절·카드의 Foraging Encounter 해결 | 채집 조우 | Foraging/Encounter engines | Partial | `gameplayEngine.test.ts [FORAGE-006]` | 정확한 행 선택과 pending 처리는 구현됐으나 일부 printed effect는 manual이다. |
...
| LEAVE-006 | p37 | 필요한 준비를 마치면 다음 Move로 복귀 | 떠나기·이동 | state transition | Partial | 없음 | 수동 중단/Delve 상태와 결합하면 의무 상태를 우회하거나 교착될 수 있다. |
...
| DOWNTIME-002 | p40 | Rumour → 평판 15 이상이며 City에서 종료; 4장으로 종류/방향/지역/거리, 불가능 조합 재추첨 | 휴식기 | `rumourEngine.ts`, canonical map selector | Exact | `phase4Engine.test.ts [DOWNTIME-002]` | 평판·City·휴식기 1회 조건을 강제하고 실제 graph의 방향·지역·Path 후보만 허용하며 불가능한 조합은 재추첨한다. |
| DOWNTIME-003 | p41 | General Practice → 질환 Tag 하나를 캠페인에 맞게 바꿈 | 휴식기 | `downtimeEngine.ts`, general practice UI | Partial | `gameplayEngine.test.ts [DOWNTIME-001]` | 5 Trinket과 1회 제한은 엔진 처리하지만 장기 Tag 변경은 아직 수동 journal 기록이다. |
| DOWNTIME-004 | p41 | Replenish → 가방을 규칙대로 채움 | 휴식기 | Downtime engine + replenish adapter | Partial | `gameplayEngine.test.ts [DOWNTIME-001]` | 정확히 한 활동으로 제한되지만 보충 선택과 지역 가용성 적용은 수동 adapter다. |
| DOWNTIME-005 | p41-42 | Explore/Work on Yourself/Reconnect → 각 조건·거리·효과를 한 번 적용 | 휴식기 | Downtime engine + activity adapters | Partial | `gameplayEngine.test.ts [DOWNTIME-001]` | 반복 실행은 차단됐지만 Explore 거리 등 선택 결과는 아직 수동 입력이다. |
...
| DOWNTIME-007 | p42-43 | Lend Paw와 Commission Wagon → 명시 비용·조건·완료 시점 적용 | 휴식기 | `downtimeEngine.ts` | Partial | `gameplayEngine.test.ts [DOWNTIME-007]` | Lend Paw +5와 City/20 Trinket 조건은 실행되며 판본 내 Wagon 비용 충돌 판정은 유지한다. |
| CLINIC-001 | p44-45 | 4계절 완료 후 Wild에서 치료 성공; 15 Trinket 지불해 Clinic 설립 | 진료소 | clinic panel/handler | Partial | 없음 | UI 조건은 유사하나 핸들러 자체 검증이 약하고 계절 완료 계산이 수동 일수에 의존한다. |
...
| CLINIC-005 | p46-47 | 10개 Agenda의 비용·조건·효과를 정확히 적용 | 진료소 | agenda handlers | Partial | 없음 | 여러 효과가 텍스트/수동이고 Gardens의 식물 선택이 전역 하나로 제한된다. |
...
| ALMANACK-004 | p58-61 | Guild Service 17종의 조건·비용·효과를 정확히 적용 | 서비스 | service data/handlers | Partial | 없음 | 16종이며 Send Package가 없고 여러 효과가 자유 텍스트/즉시 지급으로 축약됐다. |
| ALMANACK-005 | p62-65 | Tool 18종과 획득 제한·준비법·효과를 정확히 적용 | 상점·가방 | `data/tools.ts`, `toolEngine.ts` | Partial | `phase4Engine.test.ts` | 18종과 Titan Thingamabob, 준비법·무게는 복구됐으나 일부 고유 trigger의 UI 연결은 남아 있다. |
| ALMANACK-006 | p66-67 | Tool Upgrade 7종은 해당 도구와 조건을 충족할 때 고유 효과 적용 | 상점·가방 | `data/upgrades.ts`, `toolEngine.ts` | Partial | `phase4Engine.test.ts`, `phase5Engine.test.ts` | 7종과 base Tool 검증·trigger state는 구현됐으나 모든 행동 화면이 trigger resolver를 호출하지는 않는다. |
| SERVICE-001 | p59 | Forecast는 다음 3 Moves 동안 Weather Foraging Encounter의 부정적 효과를 무시 | 서비스·채집 | `serviceEngine.ts`, forecast state | Partial | `phase4Engine.test.ts [SERVICE-001]` | 원문 p59의 3 Moves를 저장·감소시키지만 모든 manual Weather effect의 자동 억제는 남아 있다. |
| SERVICE-002 | p58-61 | Shortcut/Hitch/Bridge/Survey는 지도 거리·대상 조건을 지켜 지도 상태 변경 | 서비스·지도 | service handlers | Partial | 없음 | 자유 텍스트 대상을 받아 경로 거리와 합법성을 충분히 검증하지 않는다. |
...
| WAGON-002 | p68-69 | Expansion 10종의 슬롯·비용·효과를 정확히 적용 | 왜건 | wagon data/handlers | Partial | 없음 | 10종은 있으나 Pedal/Clay Pots 등 일부 효과가 완전히 강제되지 않는다. |
...
| WAGON-004 | p68-69 | Sealed/Shadow/Experimental 등 확장 효과를 해당 판정 시점에만 적용 | 이동·조우 | wagon flags | Partial | 없음 | 일부 flag는 연결됐으나 수동 조우 처리 때문에 결과 보장이 없다. |
| COMPANION-001 | p70-71 | Companion 9종은 고유 조건·주기·효과만 제공 | 동료 | companion data/handlers | Partial | 없음 | 9종은 있으나 여러 주기/조건이 축약됐다. |
...
| COMPANION-005 | p70-71 | Beetle/Pond Skimmer 등 위치·행동 제한과 보상은 정확히 적용 | 이동·동료 | companion handlers | Partial | 없음 | 일부 수치만 있고 위치/타이밍 조건이 완전하지 않다. |
...
| BARROW-005 | p118 | Uneasy Sleep → SLEEP 요구, Timer/추격/보상 절차 | Barrow | `barrowEngine.ts` | Partial | `phase4Engine.test.ts` | canonical engine은 SLEEP 6·Timer·추격·보상을 검증하지만 전용 입력 UI가 완전하지 않다. |
| BARROW-006 | p119 | Pilfer → face 값과 합계 21 보상을 정확히 적용 | Barrow | pilfer handler | Partial | 없음 | 카드값은 맞지만 21 보상이 실제 Tool 대신 교환권 placeholder다. |
| BARROW-007 | p120-121 | Building Trust/Suitable Furnishings → 정확한 질환·재료·카드·보상 검증 | Barrow | `barrowEngine.ts` | Partial | `phase4Engine.test.ts` | canonical 질환 identity와 순서 있는 5 Reagent/M=12를 검증하지만 전용 UI는 남아 있다. |
| BARROW-008 | p122-123 | Full Bellies/Inside Job → 요구 태그·Timer·선택 분기·지도 결과 적용 | Barrow | `barrowEngine.ts` | Partial | `phase4Engine.test.ts` | Tag·Timer·warn others 분기는 구조화됐으나 일부 지도 후속은 기존 화면 adapter를 거친다. |
| BARROW-009 | p124-125 | Potent Poison → 서로 다른 7 Reagent와 카드값으로 진행 | Barrow | `barrowEngine.ts` | Partial | `phase4Engine.test.ts` | 일곱 canonical identity와 M=12를 검증하지만 전체 UI transaction 연결은 남아 있다. |
...
| TABLE-006 | p10-21, p28, p40 | 캐릭터·목표·환자·Rumour 등 d12/다중 카드 표의 항목 수와 분포 유지 | 생성·여정·환자·휴식기 | assorted data/UI | Partial | 없음 | 캐릭터/Goal 일부는 있으나 환자 2장과 Rumour 4장 표가 없다. |
...
| UX-001 | 규칙 가시성 | 강제 규칙과 선택 규칙을 명확히 구분하고 불가능 행동은 차단 | 조우·치료 | engine result flows | Partial | `gameplayEngine.test.ts [TRAVEL-009/SAVE-004]` | 핵심 흐름은 invalid/manual/resolved를 구분하지만 일부 legacy Barrow/Barter 화면은 수동 계산을 유지한다. |
