...
export const validateCanonicalData = (): ValidationReport => {
  ...
  const expectedCounts: Array<[string, number, number]> = [
    ...
    ,['Implemented Printed Effects', PRINTED_EFFECT_REGISTRY.filter(row => row.status === 'implemented').length, 29]
    ,['Manual Printed Effects', PRINTED_EFFECT_REGISTRY.filter(row => row.status === 'manual').length, 329]
