...
export const parseMechanicalEffects = (body: string): StructuredRuleEffect[] => {
  ...
  const gainRep = [...body.matchAll(/gain (\d+) reputation/gi)];
  const loseRep = [...body.matchAll(/lose (\d+) reputation/gi)];
  const decreaseTimers = [...body.matchAll(/(?:decrease|reduce) timers? by (?:an additional )?(\d+)/gi)];
  const increaseTimers = [...body.matchAll(/(?:increase|add) (?:(\d+) to (?:your )?next timer|timers? by (\d+))/gi)];
  const days = [...body.matchAll(/(?:mark (\d+) days?(?: on your calendar)?|add (\d+) days? to your calendar)/gi)];
  const gainForaging = [...body.matchAll(/gain (\d+) foraging points?/gi)];
  const loseForaging = [...body.matchAll(/lose (\d+) foraging points?/gi)];
  const trinkets = [...body.matchAll(/gain (\d+) trinkets?/gi)];
  const loseTrinkets = [...body.matchAll(/lose (\d+) trinkets?/gi)];
  const payTrinkets = [...body.matchAll(/(?:pay|trade|spend) (\d+) trinkets?/gi)];
  ...
  if (/gain (?:a |1 |an )?(?:[\w'-]+ )?trinket(?!s)/i.test(body) && trinkets.length === 0) {
  ...
  if (/leave a trinket|trade a trinket(?!s)/i.test(body) && payTrinkets.length === 0) {
    effects.push(implemented({ type: 'modifyTrinkets', amount: -1 }));
  }
  if (/\bmark a day\b/i.test(body) && days.length === 0) {
