...
const rows: AlmanackRow[] = [
  ...REAGENTS.map(row => ({ id: row.id, kind: 'reagents' as const, title: row.displayName || row.canonicalName, detail: `${row.type} · BR ${row.baseRarity} · ${row.preparations.length} preparations`, page: row.sourcePage, automated: true })),
  ...AILMENTS.map(row => ({ id: row.id, kind: 'ailments' as const, title: row.displayName || row.canonicalName, detail: `${row.severity} · Timer ${row.timer}`, page: row.sourcePage, automated: row.support === 'implemented' })),
  ...GUILD_SERVICES.map(row => ({ id: row.id, kind: 'services' as const, title: row.name, detail: `${row.provider} · ${Array.isArray(row.cost) ? row.cost.join('/') : row.cost} Trinkets`, page: row.sourcePage, automated: true })),
  ...ALMANACK_TOOLS.map(row => ({ id: row.id, kind: 'tools' as const, title: row.canonicalName, detail: `Weight ${row.weight} · ${row.cost ?? 'not purchasable'} Trinkets`, page: row.sourcePage, automated: true })),
  ...TOOL_UPGRADES.map(row => ({ id: row.id, kind: 'upgrades' as const, title: row.canonicalName, detail: `${row.baseToolId} · ${row.trigger}`, page: row.sourcePage, automated: true })),
  ...WAGON_EXPANSIONS.map(row => ({ id: row.id, kind: 'wagons' as const, title: row.canonicalName, detail: `${row.location} · ${row.cost} Trinkets`, page: row.sourcePage, automated: true })),
  ...COMPANIONS.map(row => ({ id: row.id, kind: 'companions' as const, title: row.canonicalName, detail: `${row.regions.join(', ')} · ${row.cost} Trinkets`, page: row.sourcePage, automated: true })),
  ...CLINIC_AGENDAS.map(row => ({ id: row.id, kind: 'clinics' as const, title: row.canonicalName, detail: row.requirement, page: row.sourcePage, automated: true })),
  ...ENCOUNTERS.map(row => ({ id: row.id, kind: 'encounters' as const, title: row.title, detail: `${row.encounterType} · ${row.region}`, page: row.sourcePage, automated: row.support === 'implemented' })),
  ...BARROW_DELVES.map(row => ({ id: row.id, kind: 'barrows' as const, title: row.name, detail: `${row.behemothClass} · ${row.challenge}`, page: row.sourcePage, automated: true }))
];
...
export default function AlmanackPanel({ ownedIds = [], discoveredIds = [] }: { ownedIds?: string[]; discoveredIds?: string[] }) {
  ...
  const [automation, setAutomation] = useState<'all' | 'automatic' | 'manual'>('all');
  ...
  const visible = useMemo(() => rows.filter(row => {
    ...
    if (automation === 'automatic' && !row.automated) return false;
    if (automation === 'manual' && row.automated) return false;
  ...
  return (
    <section className="almanack" aria-labelledby="almanack-title">
      ...
      <div className="almanack__controls">
        ...
        <label><span>처리 방식</span><select value={automation} onChange={event => { setAutomation(event.target.value as typeof automation); setVisibleLimit(80); }}><option value="all">전체</option><option value="automatic">자동 판정</option><option value="manual">직접 판정</option></select></label>
      </div>
      ...
      <div className="almanack__index" role="list">
        {visible.slice(0, visibleLimit).map(row => {
          ...
          return <article key={`${row.kind}:${row.id}`} className="almanack-entry" role="listitem" onClick={() => setRecent(current => [row.id, ...current.filter(id => id !== row.id)].slice(0, 6))}>
            ...
            <div className="almanack-entry__meta"><span>{owned.has(row.id) ? '현재 보유' : hiddenKnowledge ? '잠김' : '기본 공개'}</span><span>{row.automated ? '자동 판정' : '직접 판정'}</span><span>p.{row.page}</span></div>
          </article>;
        })}
      </div>
