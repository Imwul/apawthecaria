...
| Tool/Upgrade resolvers | Implemented, trigger UI partial | 18 Tool, 7 Upgrade, base Tool identity, Weight, breakage, one-shot trigger | `TOOL-001-005` |
...
| Save queue | Implemented | schema v5, local-first outbox, retry, same-revision conflict, ordered writes | `SAVE-001/004/006-008`, `OFFLINE-001-003` |
...
| `createReplacementAcquisition()` | Partial | 이름·Preparation·Rarity 12·Weight 2/3 획득 요구를 저장; 직접 생성 차단 | `REMEDY-003` |
...
- 현재 자동 완료는 안전하게 수치화된 3개 행이며 355개는 `manual`이다. Registry 등록이나 구조 필드만으로 자동 구현 완료로 간주하지 않는다.
- 원문 경고 10행은 제목·페이지·계절·상태를 다시 대조했다. 자동화가 안전하지 않은 후속 카드, 지도 표식, 아이템 선택은 구체적인 manual decision으로 남겼다.
- Ailment 11종은 일반 치료식 앞에 적용되는 구조화 예외가 추가됐지만, 나머지 서술·지도·후속 환자 결과는 manual이다.
...
- schema v4는 `pendingBarter`, `journey`, `pendingEnding`, `pendingLeaveObligation`, `pendingAlternativeAcquisition`, `patientArchive`와 Ailment effect 필드를 보존한다.
...
- 355개 printed effect row는 자동 executor가 아닌 명시적 manual 상태다. 특히 지도 생성, 후속 카드, 대상 아이템 선택, 장기 NPC 상태는 다음 전사 단계가 필요하다.
- Replacement commit resolver는 있으나 Rarity 12 Forage/Barter 성공에서 custom Inventory item으로 이어지는 전체 UI transaction이 아직 없다.
- `resolveLeave()`는 pure core와 테스트가 있으나 모든 legacy 환자 포기 UI를 하나의 transaction으로 교체하지는 못했다.
- Barrow 전용 입력과 일부 Tool/Upgrade trigger는 canonical resolver가 있어도 legacy 화면 adapter가 남는다.
- 치료 확정 전 재료 선택 draft 필드는 schema에 있지만 현재 선택 UI 전체가 복원되지는 않는다.
- same-revision 충돌은 local 보존과 알림으로 처리하며 자동 field merge는 하지 않는다.

## 다음 Phase 권장 순서

1. `PRINTED_EFFECT_STATUS.md`의 manual 행 중 수치·조건만으로 결정 가능한 항목을 원문 순서대로 executor로 이동한다.
2. Replacement Rarity 12 commit을 Forage/Barter pending UI에 연결한다.
3. Barrow 8종 전용 입력 화면을 pure resolver에 직접 연결해 legacy Delve write를 제거한다.
4. Tool/Upgrade trigger를 Travel/Forage/Treatment 화면의 공통 transaction으로 통합한다.
5. 치료 draft 복원과 나머지 legacy write adapter를 줄인다.
